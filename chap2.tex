...
@gen function step_forward(
    t::Int,
    prev_scene_graph::Union{MetaDiGraph,Nothing},
    hypers::Hypers,
)
    ...
    if t == 1
        ...
        prev_structure = get_structure(prev_scene_graph)
        structure ~ Mixture(...)
        params ~ params_dynamics(structure, prev_scene_graph, hypers)

    end

    # observation
    scene_graph = SceneGraph(structure, params)
    obs ~ noise_model(scene_graph)

    return scene_graph
end
\end{lstlisting}
\caption{Probabilistic pseudocode for generative model}
\end{algorithm}
